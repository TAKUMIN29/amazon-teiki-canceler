...
> （一覧表示・編集モーダルを開く・理由選択欄や確定ボタンの存在確認までは行いましたが、
> **実際に確定ボタンをクリックしての解約/スキップの成否は未検証**です — 誤操作を避ける
> ため意図的に一度もクリックしていません）。初めて使う際は必ず `--dry-run` で
> 「最後の確定ボタンまで到達できるか」を確認したうえで、まず1件だけ試すことを
> 強く推奨します。
...
一覧を表示したあと、商品を1件ずつ「何もしない / 次回のお届けをスキップ / 解約する」
の3択で選んでいきます。選び終わると実行予定の一覧（どれをスキップ／解約するか）を
表示して最終確認し、まとめて実行します。`--dry-run` を付けると最後の確定ボタンの
手前まで動作確認だけ行います（実際には解約/スキップしません）。
...
連続実行時の再同定・スキップと解約の混在実行など）を検証します。このモックは
